...
pub fn list_tasks(
    ...
    let items = build_task_details_batch(&conn, &items)?;
...
pub fn get_random_tasks(
    ...
    if only_weak {
        conditions.push(
            "t.id IN (
                ...
                FROM task_assessments latest
                INNER JOIN (
                    SELECT task_id, MAX(id) AS max_id
                    FROM task_assessments
                    GROUP BY task_id
                ) current ON current.max_id = latest.id
                WHERE latest.assessment = 'needs_work'
            )".to_string(),
        );
    }
    ...
    build_task_details_batch(&conn, &items)
}
...
fn build_task_detail(conn: &rusqlite::Connection, task: &Task) -> Result<TaskDetail, String> {
    ...
    let latest_assessment: Option<String> = conn.query_row(
        "SELECT assessment FROM task_assessments WHERE task_id = ?1 ORDER BY id DESC LIMIT 1",
        [task_id],
        |row| row.get(0),
    ).ok();
    ...
    for req in &requirements {
        let status: Option<String> = conn.query_row(
            "SELECT status FROM requirement_assessments WHERE task_id = ?1 AND req_index = ?2 ORDER BY id DESC LIMIT 1",
...
fn build_task_details_batch(conn: &rusqlite::Connection, tasks: &[Task]) -> Result<Vec<TaskDetail>, String> {
    if tasks.is_empty() {
        return Ok(vec![]);
    }

    // Build IN clause placeholders
    let placeholders: Vec<String> = (0..tasks.len()).map(|i| format!("?{}", i + 1)).collect();
    ...
    // 1. Batch query requirements
    let mut req_stmt = conn.prepare(&format!(
        "SELECT id, task_id, req_index, content FROM task_requirements WHERE task_id IN ({}) ORDER BY task_id, req_index",
        in_clause
    )).map_err(|e| e.to_string())?;
    let mut reqs_by_task: std::collections::HashMap<String, Vec<TaskRequirement>> = std::collections::HashMap::new();
    for row in req_stmt.query_map(params.as_slice(), |row| {
        Ok(TaskRequirement {
            id: row.get(0)?,
            task_id: row.get(1)?,
            req_index: row.get(2)?,
        ...
        reqs_by_task.entry(r.task_id.clone()).or_default().push(r);
    }

    // 2. Batch query knowledge points
    let mut kp_stmt = conn.prepare(&format!(
        "SELECT kp.id, kp.chapter_id, kp.name, kp.order_index, tkp.task_id \
         FROM knowledge_points kp \
         INNER JOIN task_knowledge_points tkp ON kp.id = tkp.knowledge_point_id \
         WHERE tkp.task_id IN ({}) ORDER BY kp.order_index",
        in_clause
    )).map_err(|e| e.to_string())?;
    let mut kps_by_task: std::collections::HashMap<String, Vec<KnowledgePoint>> = std::collections::HashMap::new();
    for row in kp_stmt.query_map(params.as_slice(), |row| {
        Ok((
            KnowledgePoint {
                id: row.get(0)?,
                chapter_id: row.get(1)?,
        ...
        let (kp, task_id) = row.map_err(|e| e.to_string())?;
        kps_by_task.entry(task_id).or_default().push(kp);
    }

    // 3. Batch query sources
    let mut src_stmt = conn.prepare(&format!(
        "SELECT id, task_id, university, year, exam_type, original_text, note FROM task_sources WHERE task_id IN ({})",
        in_clause
    )).map_err(|e| e.to_string())?;
    let mut srcs_by_task: std::collections::HashMap<String, Vec<TaskSource>> = std::collections::HashMap::new();
    for row in src_stmt.query_map(params.as_slice(), |row| {
        Ok(TaskSource {
            id: row.get(0)?,
            task_id: row.get(1)?,
            university: row.get(2)?,
        ...
        srcs_by_task.entry(s.task_id.clone()).or_default().push(s);
    }

    // 4. Batch query latest assessments
    let mut assess_stmt = conn.prepare(&format!(
        "SELECT task_id, assessment FROM task_assessments \
         WHERE id IN (SELECT MAX(id) FROM task_assessments WHERE task_id IN ({}) GROUP BY task_id)",
        in_clause
    )).map_err(|e| e.to_string())?;
    let mut assess_map: std::collections::HashMap<String, String> = std::collections::HashMap::new();
    for row in assess_stmt.query_map(params.as_slice(), |row| {
        Ok((row.get::<_, String>(0)?, row.get::<_, String>(1)?))
    }).map_err(|e| e.to_string())? {
        let (tid, assess) = row.map_err(|e| e.to_string())?;
        assess_map.insert(tid, assess);
    }

    // 5. Batch query latest answers
    let mut ans_stmt = conn.prepare(&format!(
        "SELECT task_id, answer_text FROM answer_records \
         WHERE id IN (SELECT MAX(id) FROM answer_records WHERE task_id IN ({}) GROUP BY task_id)",
        in_clause
    )).map_err(|e| e.to_string())?;
    let mut ans_map: std::collections::HashMap<String, String> = std::collections::HashMap::new();
    for row in ans_stmt.query_map(params.as_slice(), |row| {
        Ok((row.get::<_, String>(0)?, row.get::<_, String>(1)?))
    }).map_err(|e| e.to_string())? {
        let (tid, ans) = row.map_err(|e| e.to_string())?;
        ans_map.insert(tid, ans);
    }

    // 6. Batch query requirement statuses (latest per task_id + req_index)
    let mut req_status_stmt = conn.prepare(&format!(
        "SELECT task_id, req_index, status FROM requirement_assessments \
         WHERE id IN (\
           SELECT MAX(id) FROM requirement_assessments WHERE task_id IN ({}) GROUP BY task_id, req_index\
         )",
        in_clause
    )).map_err(|e| e.to_string())?;
    let mut req_status_map: std::collections::HashMap<(String, i64), String> = std::collections::HashMap::new();
    for row in req_status_stmt.query_map(params.as_slice(), |row| {
        Ok((
            row.get::<_, String>(0)?,
            row.get::<_, i64>(1)?,
            row.get::<_, String>(2)?,
        ...
        let (tid, idx, status) = row.map_err(|e| e.to_string())?;
        req_status_map.insert((tid, idx), status);
    }

    // Assemble TaskDetail objects preserving input order
    let mut result = Vec::with_capacity(tasks.len());
    for task in tasks {
        let tid = &task.id;
        let requirements = reqs_by_task.remove(tid).unwrap_or_default();
        let knowledge_points = kps_by_task.remove(tid).unwrap_or_default();
        let sources = srcs_by_task.remove(tid).unwrap_or_default();
        let latest_assessment = assess_map.remove(tid);
        let latest_answer = ans_map.remove(tid);

        let requirement_statuses: Vec<ReqStatus> = requirements.iter().map(|req| {
            let status = req_status_map
                .remove(&(tid.clone(), req.req_index))
